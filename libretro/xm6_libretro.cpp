...
static int g_bass_enhancer_level = 0;
...
static std::vector<uint32_t> g_scaled_video_buffer;
...
struct xm6_api_t {
  ...
  int (XM6CORE_CALL *set_bass_enhancer_level)(XM6Handle handle, int level) = nullptr;
...
static bool load_xm6_api()
{
  ...
  g_xm6.set_bass_enhancer_level = xm6_set_bass_enhancer_level;
  ...
  load_optional_symbol(&g_xm6.set_bass_enhancer_level, "xm6_set_bass_enhancer_level");
...
static void apply_runtime_core_options()
{
  ...
  if (g_xm6.set_bass_enhancer_level) {
    g_xm6.set_bass_enhancer_level(g_xm6_handle, g_bass_enhancer_level);
  }
...
static void apply_core_option_values()
{
  ...
  var.key = "xm6_bass_enhancer";
  if (g_environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE, &var) && var.value) {
    g_bass_enhancer_level = std::atoi(var.value);
    if (g_bass_enhancer_level < 0) {
      g_bass_enhancer_level = 0;
    } else if (g_bass_enhancer_level > 100) {
      g_bass_enhancer_level = 100;
    }
  }
  ...
  if (version >= 2) {
    ...
    static retro_core_option_v2_definition defs[] = {
      ...
      },
      {
        ...
        "Bass Enhancer",
        nullptr,
        "Apply a subharmonic bass enhancer to the final mix. 0 is bypass.",
        nullptr,
        "sound",
        {
          { "0", nullptr },
  ...
  unsigned *out_stride_pixels)
{
  ...
  if (h_factor <= 1 && v_factor <= 1 && !weave_lowres_interlace) {
  ...
  const unsigned scaled_width = frame.width * h_factor;
  const unsigned scaled_height = frame.height * v_factor;
  if (scaled_width == 0 || scaled_height == 0 || scaled_width > 1024 || scaled_height > 1024) {
    return false;
  }

  const size_t pixels = static_cast<size_t>(scaled_width) * static_cast<size_t>(scaled_height);
  g_scaled_video_buffer.resize(pixels);

  for (unsigned y = 0; y < frame.height; ++y) {
    const uint32_t *src = frame.pixels_argb32 + static_cast<size_t>(y) * frame.stride_pixels;
    const uint32_t *src_next = (y + 1 < frame.height) ? src + frame.stride_pixels : src;
    for (unsigned vy = 0; vy < v_factor; ++vy) {
      uint32_t *dst = g_scaled_video_buffer.data() +
        (static_cast<size_t>(y) * v_factor + vy) * scaled_width;
      for (unsigned x = 0; x < frame.width; ++x) {
        uint32_t c = src[x];
        if (weave_lowres_interlace && v_factor > 1 && vy == 1) {
          c = src_next[x];
        }
  ...
  *out_pixels = g_scaled_video_buffer.data();
  *out_width = scaled_width;
  *out_height = scaled_height;
  *out_stride_pixels = scaled_width;
  return true;
}
  ...
  if (g_environ_cb) {
    ...
    if (g_environ_cb(RETRO_ENVIRONMENT_GET_VARIABLE_UPDATE, &updated) && updated) {
      ...
      const int old_adpcm_volume = g_adpcm_volume;
	      ...
	      const int old_bass_enhancer_level = g_bass_enhancer_level;
      ...
      if (old_bass_enhancer_level != g_bass_enhancer_level) {
        apply_runtime_core_options();
        core_log(RETRO_LOG_INFO,
                 "[xm6-libretro] Bass Enhancer level %d",
                 g_bass_enhancer_level);
      }
