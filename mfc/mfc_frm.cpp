...
void FASTCALL CFrmWnd::InitCmd(LPCTSTR lpszCmd)
{
	...
	for (i=0; i<2; i++) {
		...
		// ç≈èâÇ™É_ÉuÉãÉNÉHÅ[ÉgÇ»ÇÁÅAéüÇÃÉNÉHÅ[ÉgÇíTÇ∑
		if (lpszCurrent[0] == _T('\x22')) {
			lpszNext = _tcschr(lpszCurrent + 1, _T('\x22'));
			if (!lpszNext) {
				// ëŒâûÇ∑ÇÈÉ_ÉuÉãÉNÉHÅ[ÉgÇ™å©Ç¬Ç©ÇÁÇ»Ç¢
				return;
			}
...
LONG CFrmWnd::OnKick(UINT /*uParam*/, LONG /*lParam*/)
{
	...
	// ç≈ëÂâªéwíËÇ≈Ç†ÇÍÇŒÅAñﬂÇµÇΩå„Ç…ÅAÉtÉãÉXÉNÉäÅ[Éì
	bFullScreen = FALSE;
	if (IsZoomed()) {
	...
	// ÉEÉCÉìÉhÉEà íuÇÉåÉWÉÖÅ[ÉÄ
	bFullScreen = RestoreFrameWnd(bFullScreen);
	if (bFullScreen) {
		// ç≈ëÂâªéwíËÇ©ÅAëOâÒé¿çséûÇ…ÉtÉãÉXÉNÉäÅ[Éì
		PostMessage(WM_COMMAND, IDM_FULLSCREEN);
	}

	// ÉfÉBÉXÉNÅEÉXÉeÅ[ÉgÇÉåÉWÉÖÅ[ÉÄ
	RestoreDiskState();

	// ñ≥å¿ÉãÅ[Év
	nIdle = 0;
	while (!m_bExit) {
		// ÉÅÉbÉZÅ[ÉWÉ`ÉFÉbÉNÅïÉ|ÉìÉv
		if (::PeekMessage(&msg, NULL, 0, 0, PM_NOREMOVE)) {
			if (!AfxGetApp()->PumpMessage()) {
				::PostQuitMessage(0);
				return 0;
			}
			// continueÇ∑ÇÈÇ±Ç∆Ç≈ÅAWM_DESTROYíºå„ÇÃm_bExitÉ`ÉFÉbÉNÇï€èÿ
			continue;
		}
...
void FASTCALL CFrmWnd::CleanSub()
{
	...
	// ÉXÉPÉWÉÖÅ[ÉâÇ™é¿çsÇÇ‚ÇﬂÇÈÇ‹Ç≈ë“Ç¬
	for (i=0; i<8; i++) {
		::LockVM();
		::UnlockVM();
	}

	// ÉXÉPÉWÉÖÅ[ÉâÇí‚é~(CScheduler)
	if (m_nStatus == 0) {
		GetScheduler()->Stop();
	}

	// ÉRÉìÉ|Å[ÉlÉìÉgÇçÌèú
	pComponent = m_pFirstComponent;
	while (pComponent) {
		pComponent->Cleanup();
		pComponent = pComponent->GetNextComponent();
...
void CFrmWnd::RestoreDiskState()
{
	...
	// ÉXÉeÅ[ÉgÇ™éwíËÇ≥ÇÍÇƒÇ¢ÇÍÇŒÅAÇ±ÇÍÇêÊÇ…çsÇ§
	if (config.resume_state) {
		...
		if (config.resume_xm6) {
			...
			if (OnOpenPrep(path)) {
				// ÉIÅ[ÉvÉìÉTÉu
				if (OnOpenSub(path)) {
					// ê¨å˜Ç»ÇÃÇ≈ÅAÉfÉtÉHÉãÉgÉfÉBÉåÉNÉgÉäÇæÇØèàóù
					if (config.resume_dir) {
						Filepath::SetDefaultDir(config.resume_path);
					}

					// Ç±ÇÍà»ç~ÇÕèàóùÇµÇ»Ç¢(FD, MO, CDÇÃÉAÉNÉZÉXíÜÇ…ÉZÅ[ÉuÇµÇΩèÍçá)
					return;
				}
			}
		}
	...
	if (config.resume_fd) {
		for (nDrive=0; nDrive<2; nDrive++) {
			// ÉfÉBÉXÉNë}ì¸Ç≥ÇÍÇƒÇ¢ÇΩÇ©
			if (!config.resume_fdi[nDrive]) {
				// ÉfÉBÉXÉNë}ì¸Ç≥ÇÍÇƒÇ¢Ç»Ç¢ÅBÉXÉLÉbÉv
				continue;
			}
			...
			// VMÉçÉbÉNÇçsÇ¢ÅAÉfÉBÉXÉNäÑÇËìñÇƒÇééÇ›ÇÈ
			::LockVM();
			bResult = m_pFDD->Open(nDrive, path, config.resume_fdm[nDrive]);
			::UnlockVM();

			// äÑÇËìñÇƒÇ≈Ç´Ç»ÇØÇÍÇŒÉXÉLÉbÉv
			if (!bResult) {
				continue;
			}
...
void CFrmWnd::OnActivate(UINT nState, CWnd *pWnd, BOOL bMinimized)
{
	...
	// èâä˙âªçœÇ›Ç»ÇÁ
	if (m_nStatus == 0) {
		// ÉCÉìÉvÉbÉgÅAÉXÉPÉWÉÖÅ[ÉâÇ÷í ím
		pInput = GetInput();
		pScheduler = GetScheduler();
		if (pInput && pScheduler) {
			// WA_INACTIVEÇ©ç≈è¨âªÇ»ÇÁÅAÉfÉBÉZÅ[ÉuÉã
			if ((nState == WA_INACTIVE) || bMinimized) {
				// ì¸óÕéÛÇØïtÇØÇ»Ç¢ÅAí·ë¨é¿çs
				pInput->Activate(FALSE);
				pScheduler->Activate(FALSE);

				// É}ÉEÉXÉÇÅ[ÉhOFF(POPUPÉEÉBÉìÉhÉEëŒçÙ)
				if (pInput->GetMouseMode()) {
					OnMouseMode();
				}
			}
			else {
				// ì¸óÕéÛÇØïtÇØÇÈÅAí èÌé¿çs
...
void CFrmWnd::OnEnterMenuLoop(BOOL bTrackPopup)
{
	...
	// É}ÉEÉXÉÇÅ[ÉhFALSE(É}ÉEÉXÇ≈ÉÅÉjÉÖÅ[Ç™ëÄçÏÇ≈Ç´ÇÈÇÊÇ§Ç…)
	if (pInput->GetMouseMode()) {
		OnMouseMode();
	}
...
void CFrmWnd::OnParentNotify(UINT message, LPARAM lParam)
{
	...
	if ((message == WM_MBUTTONDOWN) && (m_nStatus == 0)) {
		...
		pInput = GetInput();
		if (pInput) {
			// É}ÉEÉXñ≥å¯Ç»ÇÁóLå¯Ç…Ç∑ÇÈÅBãtÇÕÇµÇ»Ç¢
			if (!pInput->GetMouseMode()) {
				// ê›íËÇ≈"íÜÉ{É^Éìã÷é~"Ç…Ç≥ÇÍÇƒÇ¢Ç»Ç¢Ç±Ç∆Ç™èåè
				if (m_bMouseMid) {
					OnMouseMode();
				}
			}
...
void CFrmWnd::OnContextMenu(CWnd * /*pWnd*/, CPoint pos)
{
	...
	if ((pos.x == -1) && (pos.y == -1)) {
		...
		// É}ÉEÉXÉÇÅ[ÉhÇ≈Ç†ÇÍÇŒÅAâèú(ÉLÅ[É{Å[ÉhÇ©ÇÁÇÃÉÅÉjÉÖÅ[ãNìÆ)
		if (GetInput()->GetMouseMode()) {
			OnMouseMode();
		}
	}
	else {
		// É}ÉEÉXÉÇÅ[ÉhÇ≈Ç†ÇÍÇŒÅAñ≥éã(É}ÉEÉXÇ©ÇÁÇÃÉÅÉjÉÖÅ[ãNìÆ)
		if (GetInput()->GetMouseMode()) {
			return;
		}
	}
...
{
	// ïWèÄÉEÉBÉìÉhÉEà íuÇÉTÉ|Å[Ég
	if ((nID & 0xfff0) == IDM_STDWIN) {
		InitPos(TRUE);
		return;
	}

	// ç≈ëÂâªÇÕÉtÉãÉXÉNÉäÅ[Éì
...
LRESULT CFrmWnd::OnShellNotify(UINT uParam, LONG lParam)
{
	...
	// Windows NTÇ©
	if (::IsWinNT()) {
		// Windows2000/XPÇÃèÍçáÅASHChangeNotification_LockÇ≈ÉçÉbÉNÇ∑ÇÈ
		...
		// Windows9xÇÃèÍçáÅApidlsÇ∆nEventÇÕuParam,lParamÇ©ÇÁíºê⁄ìæÇÈ
		pidls = (LPITEMIDLIST*)uParam;
		nEvent = lParam;
		hLock = NULL;
	}

	// é¿çsíÜÇ≈ÅACHostÇ™Ç†ÇÍÇŒÅAí ím
	if (m_nStatus == 0) {
		pHost = GetHost();

#if 1
		// WindrvÇ™Ç‹Çæïsà¿íËÇÃÇΩÇﬂÅAé¿ç€Ç…EnableÇ…Ç≥ÇÍÇƒÇ¢Ç»Ç¢èÍçáÇÕâΩÇ‡ÇµÇ»Ç¢(version2.04)
		{
			Config config;
			GetConfig()->GetConfig(&config);
	...
	// NTÇÃèÍçáÅASHCnangeNotifcation_UnlockÇ≈ÉAÉìÉçÉbÉNÇ∑ÇÈ
	if (::IsWinNT()) {
		ASSERT(hLock);
...
void CFrmWnd::GetMessageString(UINT nID, CString& rMessage) const
{
	...
	// Ç±Ç±Ç‹Ç≈Ç≈ämíËÇµÇƒÇ¢Ç»ÇØÇÍÇŒÅAäÓñ{ÉNÉâÉX
	if (!bValid) {
		CFrameWnd::GetMessageString(nID, rMessage);
	}

	// åãâ ÇInfoÇ÷íÒãü(ì‡ïîï€éùóp)
	pInfo = GetInfo();
	if (pInfo) {
		pInfo->SetMessageString(rMessage);
	}

	// åãâ ÇÉXÉeÅ[É^ÉXÉrÉÖÅ[Ç÷íÒãü
	if (m_pStatusView) {
...
void FASTCALL CFrmWnd::ShowStatus()
{
	...
	// ÉtÉãÉXÉNÉäÅ[ÉìÇ©
	if (m_bFullScreen) {
		// ÉXÉeÅ[É^ÉXÉoÅ[ÇÕèÌÇ…îÒï\é¶
		ShowControlBar(&m_StatusBar, FALSE, FALSE);

		// ÉXÉeÅ[É^ÉXÉoÅ[ï\é¶Ç©
		if (m_bStatusBar) {
			// ÉXÉeÅ[É^ÉXÉrÉÖÅ[Ç™ë∂ç›ÇµÇ»ÇØÇÍÇŒ
			if (!m_pStatusView) {
				// çÏê¨ÇµÇƒ
				CreateStatusView();
			...
			// ÉXÉeÅ[É^ÉXÉrÉÖÅ[Ç™ë∂ç›ÇµÇƒÇ¢ÇÍÇŒ
			if (m_pStatusView) {
				// çÌèúÇµÇƒ
				DestroyStatusView();
	...
	// ÉXÉeÅ[É^ÉXÉrÉÖÅ[ÇÕÉtÉãÉXÉNÉäÅ[ÉìêÍópÇ»ÇÃÇ≈ÅAçÌèú
	if (m_pStatusView) {
		DestroyStatusView();
		RecalcLayout();
	}

	// ÉEÉBÉìÉhÉEÇ»ÇÃÇ≈ÅAShowControlBarÇ≈êßå‰
...
void FASTCALL CFrmWnd::CreateStatusView()
{
	...
	if (m_bStatusBar) {
		// ÉXÉeÅ[É^ÉXÉrÉÖÅ[çÏê¨(çƒîzíuÇÕçsÇÌÇ»Ç¢)
		m_pStatusView = new CStatusView;
		if (m_pStatusView->Init(this)) {
			// çÏê¨ê¨å˜
			pInfo = GetInfo();
			if (pInfo) {
				// InfoÇ™ë∂ç›Ç∑ÇÈÇÃÇ≈ÅAÉXÉeÅ[É^ÉXÉrÉÖÅ[çÏê¨Çí ím
...
void FASTCALL CFrmWnd::DestroyStatusView()
{
	...
	// óLå¯Ç»ÉXÉeÅ[É^ÉXÉrÉÖÅ[Ç™ë∂ç›Ç∑ÇÈèÍçáÇÃÇ›
	if (m_pStatusView) {
		// InfoéÊìæ
		pInfo = GetInfo();
		if (pInfo) {
			// InfoÇ™ë∂ç›Ç∑ÇÈÇÃÇ≈ÅAÉXÉeÅ[É^ÉXÉrÉÖÅ[çÌèúÇí ím
			pInfo->SetStatusView(NULL);
		}

		// ÉXÉeÅ[É^ÉXÉrÉÖÅ[çÌèú(çƒîzíuÇÕçsÇÌÇ»Ç¢)
...
void FASTCALL CFrmWnd::RecalcStatusView()
{
	...
	// ÉtÉåÅ[ÉÄÇÃÉTÉCÉYÇéÊìæ
	GetClientRect(&rectFrame);

	// ÉXÉeÅ[É^ÉXÉrÉÖÅ[ÇÃóLñ≥Ç≈ï™ÇØÇÈ
	if (m_pStatusView) {
		// ÉXÉeÅ[É^ÉXÉrÉÖÅ[Ç†ÇËÅBÉXÉeÅ[É^ÉXÉrÉÖÅ[ÇÃà íuÇóDêÊ
		m_pStatusView->GetWindowRect(&rectStatus);
		lStatus = rectStatus.Height();
		lDraw = rectFrame.Height() - lStatus;

		// DrawÉrÉÖÅ[ÇÃà íuÇéÊìæ
		...
		// ÉXÉeÅ[É^ÉXÉrÉÖÅ[Ç»ÇµÅBDrawÉrÉÖÅ[ÇÃÇ›
		m_pDrawView->GetWindowRect(&rectDraw);
		ScreenToClient(&rectDraw);
...
void FASTCALL CFrmWnd::ShowMenu()
{
	...
	// ïKóvÇ≈Ç†ÇÍÇŒVMÇÉçÉbÉN
	if (m_nStatus == 0) {
		::LockVM();
	}

	// åªç›ÇÃÉÅÉjÉÖÅ[ÇéÊìæ
	hMenu = ::GetMenu(m_hWnd);

	// ÉÅÉjÉÖÅ[Ç™ïsïKóvÇ»èÍçá
	...
	// ÉÅÉjÉÖÅ[Ç™ïKóvÇ»èÍçá
	if (hMenu != NULL) {
		// ÉZÉbÉgÇµÇΩÇ¢ÉÅÉjÉÖÅ[Ç∆ìØÇ∂Ç©
...
void FASTCALL CFrmWnd::ShowCaption()
{
	...
	// åªç›ÇÃÉLÉÉÉvÉVÉáÉìèÛë‘ÇéÊìæ
	dwStyle = GetStyle() & WS_CAPTION;

	// ÉLÉÉÉvÉVÉáÉìÇ™ïsïKóvÇ»èÍçá
	if (m_bFullScreen || !m_bCaption) {
		// ÉLÉÉÉvÉVÉáÉìÇ™ë∂ç›Ç∑ÇÈÇ©
		if (dwStyle) {
			// ÉLÉÉÉvÉVÉáÉìÇè¡ãé
	...
	// ÉLÉÉÉvÉVÉáÉìÇ™ïKóvÇ»èÍçá
	if (!dwStyle) {
		// ÉLÉÉÉvÉVÉáÉìÇÉZÉbÉg
