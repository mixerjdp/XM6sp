class Adpcm {
	...
	volatile unsigned char Panpot;	// latched ADPCM panpot
...
int FinishCounter;
	...
	inline void Reset();
	inline void SetPanpot(DWORD panpot);
	inline unsigned char GetPanpot() const { return Panpot; }
	inline int	GetPcm();
	inline int	GetPcm62();
...
inline void Adpcm::SetPanpot(DWORD panpot) {
	Panpot = (unsigned char)(panpot & 3u);
}


inline void Adpcm::DmaError(unsigned char errcode) {
	ErrorCode = errcode;
	TraceValue = 0xD0000000 | ((int)errcode << 16) | ((DmaReg[0x00] & 0xFF) << 8) | (DmaReg[0x07] & 0xFF);
	DebugValue = 0xE0000000 | ((int)errcode << 8) | (DmaReg[0x00] & 0xFF);
...
inline int Adpcm::DmaArrayChainSetNextMtcMar() {
	...
	*(unsigned short *)&DmaReg[0x1A] = bswapw(Btc);
	...
	unsigned char *Bar;
	Bar = reinterpret_cast<unsigned char *>(static_cast<std::uintptr_t>(x68sound_read_u32_be(&DmaReg[0x1C])));
	...
	x68sound_write_u32_be(&DmaReg[0x1C], static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(Bar)));
...
inline int	Adpcm::DmaGetByte() {
	if ((!(DmaReg[0x00]&0x08)) || (DmaReg[0x07]&0x20)) {	// ACT==0 || HLT==1 ?
		if (ErrorCode == 0) TraceValue = 0xA1000000 | ((DmaReg[0x00] & 0xFF) << 16) | ((DmaReg[0x07] & 0xFF) << 8) | (DmaReg[0x0A] & 0xFF);
		DebugValue = 0xA0000000 | ((DmaReg[0x00] & 0xFF) << 16) | ((DmaReg[0x07] & 0xFF) << 8) | (DmaReg[0x0A] & 0xFF);
		return 0x80000000;
	}
	...
	if (DmaReg[0x07] & 0x40) {	// Continue??
		...
		if (ErrorCode == 0) TraceValue = 0xA2000000 | ((DmaReg[0x00] & 0xFF) << 16) | ((DmaReg[0x07] & 0xFF) << 8) | (DmaReg[0x0A] & 0xFF);
		return 0x80000000;
	}
	}
	...
	{
		unsigned char *Mar;
		Mar = reinterpret_cast<unsigned char *>(static_cast<std::uintptr_t>(x68sound_read_u32_be(&DmaReg[0x0C])));
		int mem;
		mem = MemRead(Mar);
		if (mem == -1) {
			if (ErrorCode == 0) TraceValue = 0xA3000000 | ((DmaReg[0x00] & 0xFF) << 16) | ((DmaReg[0x07] & 0xFF) << 8) | (DmaReg[0x0A] & 0xFF);
			DmaError(0x09);	// ?????(???????/???????)
			return 0x80000000;
		}
		...
		x68sound_write_u32_be(&DmaReg[0x0C], static_cast<std::uint32_t>(reinterpret_cast<std::uintptr_t>(Mar)));
	}
...
//			}
	...
	DebugValue = 0x10000000 | ((DmaLastValue & 0xFF) << 16) | ((DmaReg[0x0A] & 0xFF) << 8) | (DmaReg[0x0C] & 0xFF);
	return DmaLastValue;
}
...
inline int Adpcm::GetPcm() {
	if (AdpcmReg & 0x80) {		// ADPCM ???
		if (ErrorCode == 0) TraceValue = 0xA4000000 | (AdpcmReg & 0xFF);
		return 0x80000000;
	}
    ...
    static int adpcm_pcm_trace_count = 0;
    if (adpcm_pcm_trace_count < 24) {
        fprintf(stderr, "[x68sound][adpcm-pcm] stage=mix ret=%d reg=$%02X rate=%d n1f=%d dmalast=$%02X csr=$%02X ccr=$%02X mtc=$%02X%02X mar=$%02X%02X%02X outp=%d inp=%d\r\n",
            (OutPcm*TotalVolume)>>8,
            (unsigned)AdpcmReg,
            RateCounter,
            N1DataFlag,
            DmaLastValue,
            (unsigned)DmaReg[0x00],
            (unsigned)DmaReg[0x07],
            (unsigned)DmaReg[0x0A],
            (unsigned)DmaReg[0x0B],
            (unsigned)DmaReg[0x0C],
            (unsigned)DmaReg[0x0D],
            (unsigned)DmaReg[0x0E],
            OutPcm,
            InpPcm);
        ++adpcm_pcm_trace_count;
    }
...
inline int Adpcm::GetPcm62() {
    ...
    static int adpcm_pcm62_trace_count = 0;
    if (adpcm_pcm62_trace_count < 24) {
        fprintf(stderr, "[x68sound][adpcm-pcm62] stage=mix ret=%d reg=$%02X rate=%d n1f=%d dmalast=$%02X csr=$%02X ccr=$%02X mtc=$%02X%02X mar=$%02X%02X%02X outp=%d inp=%d\r\n",
            (OutPcm>>9),
            (unsigned)AdpcmReg,
            RateCounter,
            N1DataFlag,
