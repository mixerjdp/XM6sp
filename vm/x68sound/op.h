...
inline void	Op::CulcArStep() {
	if (Ar != 0) {
		int ks = (Ar<<1)+(Kc>>(5-Ks));
			StatTbl[ATACK].and_ = XRTBL[ks].and_;
			StatTbl[ATACK].cmp = XRTBL[ks].and_>>1;
		if (ks < 62) {
...
inline void	Op::CulcD1rStep() {
	if (D1r != 0) {
		...
		StatTbl[DECAY].and_ = XRTBL[ks].and_;
		StatTbl[DECAY].cmp = XRTBL[ks].and_>>1;
		StatTbl[DECAY].add = XRTBL[ks].add;
	} else {
		StatTbl[DECAY].and_ = 4097;
...
inline void	Op::CulcD2rStep() {
	if (D2r != 0) {
		...
		StatTbl[SUSTAIN].and_ = XRTBL[ks].and_;
		StatTbl[SUSTAIN].cmp = XRTBL[ks].and_>>1;
		StatTbl[SUSTAIN].add = XRTBL[ks].add;
	} else {
		StatTbl[SUSTAIN].and_ = 4097;
...
inline void	Op::CulcRrStep() {
	...
	StatTbl[RELEASE].and_ = XRTBL[ks].and_;
	StatTbl[RELEASE].cmp = XRTBL[ks].and_>>1;
