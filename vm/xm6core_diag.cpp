...
static void create_vm_devices(VM* vm)
{
	new Scheduler(vm);
	new CPU(vm);
	new Keyboard(vm);
...
extern "C" XM6CORE_API int XM6CORE_CALL xm6_diag_init_probe(char* out_text, unsigned int out_text_size)
{
	...
	create_vm_devices(vm);

	if (!vm->log.Init(vm)) {
		vm->Cleanup();
		delete vm;
		return diag_set_text(out_text, out_text_size, "probe: vm->log.Init failed");
	}

	for (Device* device = vm->GetFirstDevice(); device; device = device->GetNextDevice()) {
		if (!device->Init()) {
			const char* desc = device->GetDesc() ? device->GetDesc() : "(null)";
			const unsigned long id = static_cast<unsigned long>(device->GetID());
			vm->Cleanup();
			delete vm;
			return diag_set_text(out_text, out_text_size,
								 "probe: device init failed id=%08lX desc=%s", id, desc);
		}
	}
